    ...
    <teleport to="body">
      <div class="context-menu" :style="contextMenuBehavior">
        ...
        >
          ...
          />
          <span>{{ t("common.deleteOtherTag") }}</span>
        </div>
      </div>
    </teleport>
  </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const onTabsSwitch = (tab) => {
      //Here start isClick
      // let hee = JSON.parse(localStorage.getItem('visitedRoutes'));
      // let updatePath = hee.find((item) => item.path === route.path);
      // updatePath.isClick = true;
      // store.commit('tagsView/UPDATE_VISITED_ROUTE', updatePath);
      //Here end isClick
