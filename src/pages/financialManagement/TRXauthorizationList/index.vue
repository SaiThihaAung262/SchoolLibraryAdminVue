<template>
  <div v-loading="tableList.tableData.isLoading">
    <div class="form" v-show="isShowSearch">
      <div class="form-content" v-permission="74">
    ...
    <div class="table" v-permission="74">
      <!-- <el-button @click="addNew" class="app-button" style="margin-bottom: 25px"
        ...
        添加 TRX Authorization</el-button
      > -->
      <div class="top-panel">
        <div class="left"></div>
        <div class="right">
...
export default {
  ...
  setup() {
    ...
    const tableList = useTableData(
      http.financialManagement.getTRXauthorization
    );
    ...
    const handleTransferFrom = (row) => {
      ...
      http.financialManagement
        .updateTRXBalance({ id: parseInt(row.id) })
        .then((res) => {
          if (res.data.err_code == 0) {
            ElMessage.success(res.data.err_msg);
            search();
    ...
    const handlerExport = () => {
      ...
      if (state.selectedExportLists.length != 0) {
        ...
        http.financialManagement
          .getTRXauthorization(state.param)
          .then((res) => {
            if (res.data.err_code === 0) {
              // log
